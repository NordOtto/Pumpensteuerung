...
function evaluateProgram(program, opts = {}) {
  ...
  if (!opts.forceWeather && program.weather_enabled) {
    ...
    const budget = Math.max(0, et0 - rain);
...
function startZone() {
  ...
  activeRun.zone = zone;
  const runtimeSec = Math.max(30, Math.round(zone.duration_min * 60 * activeRun.runtimeFactor));
...
function runProgram(id, opts = {}) {
  ...
  const zones = program.zones.filter(z => z.enabled);
